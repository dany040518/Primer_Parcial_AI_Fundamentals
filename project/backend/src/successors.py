...
from travel import travel_successors
...
def _drop_candidates(
    scenario: dict[str, Any], state: State, candidates: list[tuple[str, str, int]]
...
def local_successors(scenario: dict[str, Any], state: State) -> Iterator[tuple[InternalAction, State]]:
    ...
    for mtype, z, count in state.materials_ground:
        ...
        if already >= needed_material_count(scenario, mtype, state.panels_ok):
            continue
    ...
    if blockers and state.battery >= drop_cost:
        ...
        def apply_drop(over: dict[str, Any], kind: str, ident: str) -> None:
            if kind == "key":
                ...
                # volver a filtrarla la próxima vez.
            elif kind == "tool":
                over["tools_carried"] = over.get("tools_carried", state.tools_carried) - {ident}
                if tool_is_live(scenario, ident, state.panels_ok):
        ...
        drop_candidates = _drop_candidates(scenario, state, candidates)
        multi_drop_needed = False

        # SWAP(x, y): suelto x y recojo y en un solo sucesor cuando soltar un
        # solo x ya libera lo que y necesita — es la forma canónica de todo
        # DROP que genero (design.md: siempre pegado al PICKUP que lo pide),
        # así que fusionarlos no pierde ningún plan, solo salta el estado
        # intermedio. Cuando ningún x solo alcanza (y pesa más de lo que
        # cualquier candidato libera), me falta más de un DROP — ahí caigo
        # al DROP suelto de más abajo para ir liberando peso de a poco.
...
def successors(
    scenario: dict[str, Any], state: State, path_cache: dict | None = None
) -> Iterator[tuple[InternalAction, State]]:
    if path_cache is None:
        path_cache = {}
    yield from local_successors(scenario, state)
