...
- `puertas_abiertas`, `paneles_ok`, `estaciones_online`: en el papel son conjuntos de ids, igual que los de arriba. En la implementación los codifico como *bitmask* (`int`, un bit por id, según un índice fijo del escenario) en vez de `frozenset`: con pocas puertas/paneles/estaciones un `int` se hashea y compara en O(1), sin el overhead de objeto que paga un `frozenset` de strings por cada elemento. Es la misma clase de equivalencia — dos estados tienen el mismo bitmask si y solo si tienen el mismo conjunto de ids abiertos/reparados/online — solo que la codifico más barata. Es una decisión de implementación, no de modelo: el resto de esta sección aplica igual.
- `zona`: string. `batería`: entero.

El estado completo es una tupla de estos campos. Una tupla de `str`, `int`, `frozenset` y los tres bitmask es hasheable directamente, así que la uso tal cual como clave de CLOSED y como elemento de un `set`. Dos configuraciones físicamente iguales producen la misma tupla sin importar el orden en que el robot recogió o soltó cada cosa — que es justo la propiedad que necesito para que `==` y el hash coincidan con "es el mismo mundo".
...
Traté de construir, a mano, una instancia donde quitar esta condición cambiara el costo óptimo — tres veces, con diseños distintos, incluido uno con un corredor de vuelta carísimo a propósito para castigar la estrategia de "aplazar la recogida". Las tres dieron el mismo costo con y sin la condición. Después corrí un tercer modo (`weight_blind`: toda la preferencia puesta, pero sin comparar pesos) contra 30 instancias generadas al azar con pesos 1 a 3 — tampoco encontré ninguna donde costara más. Mi intuición de por qué: cada vez que un objeto vivo pesado y uno muerto liviano coexisten cargados, o el vivo se resuelve con una acción gratis antes de que llegue el conflicto, o aplazar su recogida es siempre alcanzable y nunca más caro, o si de verdad tienen que convivir en algún momento, la capacidad mínima que exige esa convivencia acota el déficit al peso del muerto y este solo ya alcanza. Cubrí esas tres rutas en los intentos, pero no demostré que sean las únicas.

**La condición se queda en el código de todos modos.** Es necesaria para que el argumento de intercambio de arriba cierre; en las instancias probadas no llega a morder, pero no está demostrado que no exista una donde sí, y prefiero una poda más conservadora que un argumento con un hueco.
